...
def _retained_step_indices(burn_in: int, thinning: int, n_chain: int) -> np.ndarray:
    """Simulation step index for each retained chain slice."""
    return np.asarray([burn_in + i * thinning for i in range(n_chain)], dtype=int)
...
def _particles_to_posterior(
    ...
    """Split flat particles into named free-RV arrays with shape (chain, draw, *event)."""
    if particles.ndim != 3:
        raise ValueError(f"particles must be 3-D (chain, draw, flat); got shape {particles.shape}.")

    free_names = {rv.name for rv in model.free_RVs}
    n_chain, n_draw, n_flat = particles.shape
    ...
    for name, shape, size, _dtype in mapper.point_map_info:
        ...
        posterior[name] = np.asarray(slab.reshape((n_chain, n_draw, *shape)), dtype=float)
        offset += size
...
def _compile_batched_logp_for_rv(model, mapper: PointMapper, rv) -> Any:
    """Compile batched elementwise logp for one observed RV; output (draw, n_obs_i)."""
...
def _eval_batched_logp_across_chains(
    batched_fn: Any,
    particles: np.ndarray,
) -> np.ndarray:
    """Evaluate batched logp over all retained chains in one compiled call."""
    n_chain, n_draw, n_flat = particles.shape
    flat = particles.reshape(n_chain * n_draw, n_flat)
    out = batched_fn(flat)
    ...
    return logp.reshape(n_chain, n_draw, *logp.shape[1:])
...
def _compute_log_likelihood(
    ...
    if len(observed_rvs) == 1:
        ...
        log_likelihood[rv.name] = _eval_batched_logp_across_chains(batched_fn, particles)
...
def _broadcast_chain_stat(values: np.ndarray, n_draw: int) -> np.ndarray:
    """Broadcast chain-only diagnostics to the standard (chain, draw) layout."""
    values = np.asarray(values, dtype=float)
    if values.ndim == 1:
        return np.broadcast_to(values[:, None], (values.shape[0], n_draw))
    return np.broadcast_to(values[:, None, ...], (values.shape[0], n_draw, *values.shape[1:]))
...
def _compute_sample_stats(
    ...
    n_chain, n_draw = particles.shape[:2]
    if n_chain == 0:
        return {}

    stats: dict[str, np.ndarray] = {
        "particle_spread": _broadcast_chain_stat(
            np.mean(np.std(particles, axis=1), axis=-1), n_draw
        ),
        "learning_rate": _broadcast_chain_stat(
            np.full(n_chain, learning_rate, dtype=float), n_draw
        ),
    }

    for name, values in posterior.items():
        stats[f"{name}_spread"] = _broadcast_chain_stat(np.std(values, axis=1), n_draw)

    if log_likelihood:
        total = sum(np.asarray(arr, dtype=float) for arr in log_likelihood.values())
        mean_score = np.mean(np.sum(total, axis=-1), axis=1)
        stats["mean_log_score"] = _broadcast_chain_stat(mean_score, n_draw)
...
def _apply_datatree_kwargs(
    ...
    *,
    ...
    kwargs["coords"]["chain"] = chain_coord
...
def pro_to_datatree(
    ...
    *,
    ...
    Particles have shape ``(chain, draw, flat)`` where ``chain`` indexes
    retained time steps and ``draw`` indexes particles in the empirical
    measure at that step.
    """
    model = modelcontext(model)
    n_chain = particles.shape[0]
    chain_coord = _retained_step_indices(burn_in, thinning, n_chain)
    ...
    log_likelihood: dict[str, np.ndarray] = {}
    if include_log_likelihood and model.observed_RVs and n_chain > 0:
        log_likelihood = _compute_log_likelihood(particles, model, mapper)
        if log_likelihood:
            data["log_likelihood"] = log_likelihood

    if include_sample_stats and n_chain > 0:
    ...
    _apply_datatree_kwargs(kwargs, datatree_kwargs, chain_coord=chain_coord)
