"""Public sampling entry for predictively oriented (PrO) posteriors (PyMC-style ``sample`` module)."""
...
def sample_posterior_predictive_pro(
    ...
    *,
    ...
    """Predictively oriented posterior predictive / out-of-sample forward sampling.
    ...
    ----------
    dt
        Predictively oriented :class:`~xarray.DataTree` from :func:`sample_pro`.
    predictions
        When ``False`` (default), populate ``posterior_predictive`` for
        in-sample posterior predictive checks. When ``True``, populate
    ...
    --------
    sample_pro
        Builds the DataTree; ``mixture_log_predictive`` holds the marginal
...
def sample_pro(
    ...
    """Run the predictively oriented (PrO) particle sampler on a PyMC model.

    Free RVs may be unconstrained or use elementwise PyMC transforms
    (``LogTransform``, ``LogOddsTransform``, ``Interval``). Coupled maps
    (e.g. simplex / Dirichlet) are not supported yet. Particles evolve in
    unconstrained ``value_vars`` with mirror-scaled diffusion (Gu & Kim 2025).
    ...
    Posterior values are packed under free-RV names (constrained scale).
    ...
    ``mean_log_score``, which averages per-particle log-score sums. Mixture
    posterior predictive draws are added separately via
    ...
    step_size
        Euler–Maruyama step size. Default ``None`` enables the functional upper
        bound step size estimator (FUSE; Sharrock and Nemeth, 2025) adaptive
        schedule. Pass a positive float (e.g. ``1e-3``) for a fixed step size
        instead.
    r_eps
    ...
    learning_rate
        Scales the log-score Wasserstein gradient flow (WGF) interaction in the
        Euler–Maruyama drift (the paper's :math:`\\lambda_n`; see Sec.
        ``Computation via Wasserstein Gradient Flows`` in McLatchie et al. 2025).
        Default ``1.0``. The compiled interaction sums over observations; this
        factor scales that sum in :func:`~pymc_prop.particles.time_step`.
    random_seed
    ...
    sample_posterior_predictive_pro
        In-sample mixture posterior predictive and out-of-sample forward
        sampling (contrast with ``mixture_log_predictive``).
